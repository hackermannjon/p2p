...
def start_moderator_room(room_name):
    rooms[room_name] = {'members': {}, 'log_file': os.path.join(LOG_DIR, f"{room_name}.log")}
...
def _member_session(conn, room_name, member_username):
    while True:
        try:
            ...
            formatted = f"[{member_username}] {msg}"
...
def show_menu(peer_port, username, peer_socket):
    while True:
        print("\n--- Salas de Chat ---")
        ...
        choice = input('> ')
        if choice == '1':
            res = send_to_tracker({'action': 'list_rooms', 'port': peer_port, 'username': username}, peer_socket)
            rooms_list = res.get('rooms', {}) if res else {}
            if not rooms_list:
                print('Nenhuma sala disponível.')
            else:
                for r, info in rooms_list.items():
                    print(f"- {r} (moderador: {info['moderator']})")
        elif choice == '2':
            room = input('Nome da sala: ')
            res = send_to_tracker({'action': 'create_room', 'room_name': room, 'port': peer_port, 'username': username}, peer_socket)
            ...
            res = send_to_tracker({'action': 'delete_room', 'room_name': room, 'port': peer_port, 'username': username}, peer_socket)
            if res and res.get('status'):
                rooms.pop(room, None)
                print('Sala removida.')
            else:
